	...
	public static void main(String[] args) {
		...
		EXIT:while(true){
			...
			default:
				...
				//进行数据书写
				Util.writerDVDInofosTOFile(dvds);
				break EXIT;
			}
			System.out.println("*******************************************");
	 ...
	 private static void initDVDArray(List<DVD> dvds){
		 //判断系统是不是第一次运行,判断当前文件是存在即可
		 if(!(Util.dvdFile.exists())){
			    dvds.add(new DVD("罗马假日",0,1,15));
				dvds.add(new DVD("风声鹤唳",1,0,12));
				dvds.add(new DVD("浪漫满屋",1,0,30));
		 }else{
			 Util.readDVDInfosFromFile(dvds);
		 }
	 }
